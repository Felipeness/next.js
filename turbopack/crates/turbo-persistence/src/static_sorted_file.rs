        ...
        while l < r {
            ...
            match comparison {
                ...
                Ordering::Equal => {
                    ...
                    // Use `l` as lower bound — binary search proved entries below
                    // `l` are strictly different.
                    for i in (l..m).rev() {
                        let GetKeyEntryResult {
                            hash,
                            key: entry_key,
                    ...
                    // Use `r` as upper bound — binary search proved entries at or
                    // above `r` are strictly different.
                    for i in (m + 1)..r {
                        let GetKeyEntryResult {
                            hash,
                            key: entry_key,
    ...
    fn lookup_fixed_key_block<K: QueryKey, const FIND_ALL: bool>(
        ...
        while l < r {
            ...
            let GetKeyEntryResult {
                ...
                ty,
                val,
            } = get_fixed_key_entry(entries, m, hash_len, key_size, value_type, stride);
